...
from scripts.seed_va_grade6 import CURRICULUM_CODE, VDOE_SOURCE, seed


def test_va_grade6_seed_is_idempotent_and_isolated():
    seed()
    seed()
    db = SessionLocal()
    try:
        ...
        assert curriculum.grade_level == "6"
...
def test_va_grade6_reuses_cross_jurisdiction_canonical_identity():
    seed()
    db = SessionLocal()
    try:
        curriculum = db.scalar(select(Curriculum).where(Curriculum.code == CURRICULUM_CODE))
        va_skill = db.scalar(select(Skill).where(
            Skill.curriculum_id == curriculum.id, Skill.code == "VA6.PFA.RATIO"))
