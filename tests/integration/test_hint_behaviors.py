    ...
    with SessionLocal() as db:
        event = db.scalar(
            ...
            .where(
                ...
                HintEvent.problem_id == problem_id,
                HintEvent.trigger == "MISCONCEPTION_JIT",
            )
...
def test_level_three_hint_forces_assisted_success_until_new_problem() -> None:
    ...
    independent = client.post(
        ...
        json={
            ...
            "answer": next_problem.canonical_answer,
            "assistance_level": 0,
        },
    )
    ...
    with SessionLocal() as db:
        independent_attempt = db.scalar(
            select(Attempt)
            .where(
                Attempt.session_id == session_id,
                Attempt.problem_id == next_problem_id,
                Attempt.is_correct.is_(True),
            )
            .order_by(Attempt.attempt_number.desc())
        )
