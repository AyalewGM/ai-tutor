...
def test_integrated_algebra1_representative_seed_is_idempotent_and_isolated():
    ...
    try:
        ...
        assert all(
            mapping.provenance_json["standards_source"] == MSDE_SOURCE
            for mapping in mappings
        )

        canonical_ids = {mapping.canonical_skill_id for mapping in mappings}
        canonical_codes = set(
            db.scalars(select(CanonicalSkill.code).where(CanonicalSkill.id.in_(canonical_ids)))
        )
        ...
        # No prerequisite edge may cross curriculum boundaries. This starter
        # slice intentionally introduces no prerequisite topology before PO review.
        edges = list(
            db.scalars(
        ...
        assert all(
            problem.solution["provenance"]["author"] == "AI Tutor curriculum team"
            for problem in problems
        )

        # Traditional Algebra I remains a distinct curriculum. Shared subject
        # matter never implies shared local skill/evidence identity.
        traditional = db.scalar(
            select(Curriculum).where(Curriculum.code == "MD_ALGEBRA_1_2026_27")
        )
        if traditional is not None:
            traditional_ids = set(
                db.scalars(select(Skill.id).where(Skill.curriculum_id == traditional.id))
            )
            assert skill_ids.isdisjoint(traditional_ids)
            traditional_canonical_ids = set(
                db.scalars(
                    select(CurriculumSkillMapping.canonical_skill_id).where(
                        CurriculumSkillMapping.skill_id.in_(traditional_ids)
                    )
                )
            )
