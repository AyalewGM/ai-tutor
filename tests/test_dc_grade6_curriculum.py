...
from scripts.seed_dc_grade6 import CURRICULUM_CODE, OSSE_SOURCE, seed


def test_dc_grade6_seed_is_idempotent_provenanced_and_isolated():
    seed()
    seed()
    db = SessionLocal()
    try:
        ...
        assert grade6.source_uri == OSSE_SOURCE
        authority = db.get(EducationAuthority, grade6.authority_id)
        ...
        assert {s.code for s in skills} == {
            "DC6.RP.RATIO", "DC6.RP.UNIT_RATE", "DC6.NS.FRACTION", "DC6.EE.EXPR", "DC6.EE.EQUATION"
        }
        ...
        for other_code in ("MCPS_MATH_6", "MCPS_MATH_7", "MCPS_MATH_8", "ON_MTH1W_2021"):
