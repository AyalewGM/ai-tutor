...
def _authority(db):
    ...
    dc = db.scalar(
        select(Jurisdiction).where(
            Jurisdiction.parent_id == us.id, Jurisdiction.code == "DC"
        )
    )
    if dc is None:
        dc = Jurisdiction(
            ...
            source_uri=OSSE_SOURCE,
        )
        db.add(dc)
        db.flush()
    authority = db.scalar(
        select(EducationAuthority).where(
            EducationAuthority.jurisdiction_id == dc.id,
            EducationAuthority.code == "OSSE",
        )
    )
    if authority is None:
        authority = EducationAuthority(
            jurisdiction_id=dc.id,
            code="OSSE",
            name="Office of the State Superintendent of Education",
            authority_type="STATE_AGENCY",
            source_uri=OSSE_SOURCE,
            provenance_json={"role": "DC mathematics standards authority"},
        )
        db.add(authority)
        db.flush()
    return authority
    ...
    if mapping is None:
        db.add(
            CurriculumSkillMapping(
                ...
                provenance_json={
                    "basis": "AI Tutor reviewed DC Algebra I mapping",
                    "standards_authority": (
                        "Office of the State Superintendent of Education"
                    ),
                    "standards_source": OSSE_SOURCE,
                    "local_implementation_context": DCPS_CONTEXT,
    ...
    try:
        ...
        if curriculum is None:
            curriculum = Curriculum(
                ...
                name="District of Columbia Algebra I — CCSS-M",
        ...
        eq = _skill(
            ...
            "DCA1.LINEAR.EQ",
            "Linear Equations",
            "Solve one-variable linear equations and justify equivalent steps.",
            2,
        ...
        fn = _skill(
            ...
            "DCA1.LINEAR.FN",
            "Linear Functions",
            "Represent linear relationships using equations, tables, and rates of change.",
            3,
        ...
        slope = _skill(
            db,
            curriculum,
            "DCA1.LINEAR.FN.SLOPE",
            "Slope-Intercept Form",
            "Write linear equations in y = mx + b form.",
            3,
        ...
        for args in [
            ...
            (
                fn,
                "A taxi charges $4 plus $2 per mile. Write the cost function for x miles.",
                "2x + 4",
                2,
            ),
            (
